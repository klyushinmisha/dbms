	...
	execErr := utils.FileScopedExec("data.bin", func(dataFile *os.File) error {
		return utils.FileScopedExec("log.bin", func(logFile *os.File) error {
			...
			return func() error {
				...
				for _, k := range keysToDelete {
					foundPtr, err := tree.Find(k)
					delPtr, err := tree.Delete(k)
					if err != nil {
						log.Panic(err)
					}
					_, err = tree.Find(k)
					...
					if foundPtr != delPtr {
						log.Panicf("Values mismatch during delete %v != %v", foundPtr, delPtr)
					}
					if keysMap[k] != delPtr {
						log.Panicf("Values mismatch during insert %v != %v", keysMap[k], delPtr)
					}
